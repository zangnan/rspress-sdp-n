...
> 如果你只关心"怎么把前端跑起来"，看 [快速开始](/sdp/env_init.html) 就够了 —— 本页是给**需要理解、改动、扩展后端**的人看的。
...
1. 浏览器打开 [http://localhost:7001/swagger-ui.html](http://localhost:7001/swagger-ui.html) —— 应能看到所有接口
...
> ⚠️ **端口说明：** 后端默认 **7001**；前端 [env_init.mdx](/sdp/env_init.html) 默认配的 `API_URL_BASE_NODE=http://localhost:8080` 是个**占位/历史值**，并不是后端真实端口。前后端联通前，请按 [Q&A · 改变后端接口地址](/sdp/qa.html#改变后端接口地址) 把前端配置指到 `http://localhost:7001`（或通过反代统一为 8080）。
...
| 你想做的事 | 看哪里 |
| --- | --- |
| 跑通前后端联调 | [快速开始](/sdp/env_init.html) + [Q&A · 改变后端接口地址](/sdp/qa.html#改变后端接口地址) |
